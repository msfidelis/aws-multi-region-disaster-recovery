...
resource "aws_iam_role" "ecs_execution_role" {
  name               = format("%s-%s-%s-role", var.cluster_name, data.aws_region.current.name, var.service_name)
  assume_role_policy = file(format("%s/templates/ecs-task-execution-role.json", path.module))
}

# Cluster Execution Policy
resource "aws_iam_role_policy" "ecs_execution_role_policy" {
  name   = format("%s-%s-%s-policy", var.cluster_name, data.aws_region.current.name, var.service_name)
